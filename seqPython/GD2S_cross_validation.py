# -*- coding: utf-8 -*-
"""
Created on Sun Jan 13 21:17:24 2019
...
def evalOneMax(individual):
    ...
    for i in range(len(trainLis)):
        ...
        for key in dict.keys(d2scountLis[0]):
            tmp_sim=tmp_sim+d2scountLis[trainLis[i][0]][key]*d2scountLis[trainLis[i][1]][key]*weight2M[key]*weight[0]/math.sqrt(d2scountLis[trainLis[i][0]][key]**2+d2scountLis[trainLis[i][1]][key]**2+np.spacing(1))
        for key in dict.keys(d3scountLis[0]):
            tmp_sim=tmp_sim+d3scountLis[trainLis[i][0]][key]*d3scountLis[trainLis[i][1]][key]*weight3M[key]*weight[1]/math.sqrt(d3scountLis[trainLis[i][0]][key]**2+d3scountLis[trainLis[i][1]][key]**2+np.spacing(1))
        for key in dict.keys(d4scountLis[0]):
            tmp_sim=tmp_sim+d4scountLis[trainLis[i][0]][key]*d4scountLis[trainLis[i][1]][key]*weight4M[key]*weight[2]/math.sqrt(d4scountLis[trainLis[i][0]][key]**2+d4scountLis[trainLis[i][1]][key]**2+np.spacing(1))    
        for key in dict.keys(d5scountLis[0]):
            tmp_sim=tmp_sim+d5scountLis[trainLis[i][0]][key]*d5scountLis[trainLis[i][1]][key]*weight5M[key]*weight[3]/math.sqrt(d5scountLis[trainLis[i][0]][key]**2+d5scountLis[trainLis[i][1]][key]**2+np.spacing(1))
        for key in dict.keys(d6scountLis[0]):
            tmp_sim=tmp_sim+d6scountLis[trainLis[i][0]][key]*d6scountLis[trainLis[i][1]][key]*weight6M[key]*weight[4]/math.sqrt(d6scountLis[trainLis[i][0]][key]**2+d6scountLis[trainLis[i][1]][key]**2+np.spacing(1)) 
        sim.append(tmp_sim)               
...
#    name="human_HBB"
   ...
   ## 获取kmer 概率集合
    ...
    #######d2s d2star特征--------------------
    d2scountLis=sq.getD2SMul_SeqCount(datasets,2,0,True,d2dic,kmer_pro2)
    d3scountLis=sq.getD2SMul_SeqCount(datasets,3,0,True,d3dic,kmer_pro3)
    d4scountLis=sq.getD2SMul_SeqCount(datasets,4,0,True,d4dic,kmer_pro4)
    d5scountLis=sq.getD2SMul_SeqCount(datasets,5,0,True,d5dic,kmer_pro5)    
    d6scountLis=sq.getD2SMul_SeqCount(datasets,6,0,True,d6dic,kmer_pro6)
    ...
    for  o in range(num):
        ...
        for i in range(len(testLis)):
            tmp_sim=0
            for key in dict.keys(d2scountLis[0]):
                tmp_sim=tmp_sim+d2scountLis[testLis[i][0]][key]*d2scountLis[testLis[i][1]][key]*weight2M[key]*w[0]/math.sqrt(d2scountLis[testLis[i][0]][key]**2+d2scountLis[testLis[i][1]][key]**2+np.spacing(1))
            for key in dict.keys(d3scountLis[0]):
                tmp_sim=tmp_sim+d3scountLis[testLis[i][0]][key]*d3scountLis[testLis[i][1]][key]*weight3M[key]*w[1]/math.sqrt(d3scountLis[testLis[i][0]][key]**2+d3scountLis[testLis[i][1]][key]**2+np.spacing(1))
            for key in dict.keys(d4scountLis[0]):
                tmp_sim=tmp_sim+d4scountLis[testLis[i][0]][key]*d4scountLis[testLis[i][1]][key]*weight4M[key]*w[2]/math.sqrt(d4scountLis[testLis[i][0]][key]**2+d4scountLis[testLis[i][1]][key]**2+np.spacing(1))
            for key in dict.keys(d5scountLis[0]):
                tmp_sim=tmp_sim+d5scountLis[testLis[i][0]][key]*d5scountLis[testLis[i][1]][key]*weight5M[key]*w[3]/math.sqrt(d5scountLis[testLis[i][0]][key]**2+d5scountLis[testLis[i][1]][key]**2+np.spacing(1))
            for key in dict.keys(d6scountLis[0]):
                tmp_sim=tmp_sim+d6scountLis[testLis[i][0]][key]*d6scountLis[testLis[i][1]][key]*weight6M[key]*w[4]/math.sqrt(d6scountLis[testLis[i][0]][key]**2+d6scountLis[testLis[i][1]][key]**2+np.spacing(1))
            sim.append(tmp_sim)     
        auc=roc_auc_score(testlabel, sim)    
        aucLis.append(auc)
        print("GA_D2s第",o,"次auc的值",auc)
    su=0
    for i in range(len(aucLis)):
        su=su+aucLis[i]
    avgAuc=su/num
    print("GA_D2S平均auc",avgAuc)
